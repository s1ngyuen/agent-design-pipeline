...
TIME BUDGET — read this before you start: this request runs inside a hard 60-second server timeout. If you run out the clock mid-research, the caller gets NOTHING — not a partial answer, a total failure. A fast, reasonably-evidenced estimate beats a maximally-thorough one that never returns. Budget roughly 3-6 total tool calls (web_search + web_fetch combined) and stop researching once you have enough to answer, even if you could theoretically dig deeper.

Research method:
1. Start with web_search for completed/sold eBay listings of the EXACT card (same player, set, parallel, print run, grade if slabbed) — search result snippets usually show price and title, often enough on their own without a follow-up fetch. Use web_fetch only when you genuinely need to open a specific page to confirm sale price, date, or sold-vs-active status.
2. 130point.com, SportsCardsPro/PriceCharting, and PSA's Auction Prices Realized (APR) are useful secondary sources IF they load quickly — 130point.com and SportsCardsPro in particular are known to sometimes block automated fetches outright. Try at most one search/fetch per secondary source; if it's slow, blocked, or unhelpful, move on immediately rather than retrying it. eBay sold listings alone are a perfectly sufficient basis for an estimate.
3. If direct comps are thin or absent (expected for low-print-run parallels or recent releases), search for 2-3 reference players (not more) in the same set/product at a similar rarity/print-run tier and use their sold prices to triangulate a value via cross-player/tier-curve reasoning. Mark these as "reference_player" entries, not "direct_comp".
4. Once you have enough evidence (often just 1-2 searches for a well-known recent product), stop and produce your final structured answer. Every reference_breakdown entry must cite what you found: a label describing the item, the source you found it on, the ACTUAL URL of the specific listing/page you found it at (from your web_search/web_fetch results), the price (or null if you're citing an active listing purely for caveat context — in which case sale_date must also be null), the sale date if known, a weight (0-1) reflecting how much it influenced your final range, and a note. Only set url to null if you genuinely cannot attribute a specific page — never fabricate or guess a URL, and never invent a plausible-looking one just to fill the field.
5. If web search turns up thin or no usable data, say so honestly in caveats rather than guessing — do not fabricate comps or prices, and do not burn your time budget searching indefinitely hoping for more.
6. Set divergence_flag.flagged = true (with a reason) when your direct comps and reference-player triangulation meaningfully disagree, or when you have very few data points and the range is more speculative than usual.
