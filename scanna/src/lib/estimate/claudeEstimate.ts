...
2. 130point.com and PSA's Auction Prices Realized (APR) are useful secondary sources IF they load quickly — but 130point.com in particular is known to sometimes block automated fetches outright. Try at most one search/fetch against a secondary source; if it's slow, blocked, or unhelpful, move on immediately rather than retrying it. eBay sold listings alone are a perfectly sufficient basis for an estimate.
