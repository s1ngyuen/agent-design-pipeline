...
> **Scope:** This workflow applies only when reproducing an existing design from a reference image. For original builds from a project brief, follow the agent pipeline in `CLAUDE.md` instead.
...
2. **Screenshot** the rendered page using Playwright (Python) or Puppeteer. If the page has distinct sections, capture those individually too.
...
## File Organisation
- All reference screenshots and build comparison images go in a `references/` subfolder inside the project folder — never in the root of the website folder.
- Example: `da-climb/references/ref_home.png`, not `da-climb/ref_home.png`
